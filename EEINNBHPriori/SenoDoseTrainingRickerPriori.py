...
with tf.device('/cpu:0'):
    class PhysicsInformedNN:
        def __init__(self, Ds_train, Rs_train, x_f, d_f, lb, ub, layers, layers_h, sf):
            ...
            bound_lamda = [30, 50]
            bound_rho = [0.01, 1]
            bound_theta = [0.01, 0.9]

            self.lamda = bound_lamda[0] + (bound_lamda[1] - bound_lamda[0]) * tf.sigmoid(tf.Variable([0.3], dtype=tf.float32, trainable=True))
            self.rho = bound_rho[0] + (bound_rho[1] - bound_rho[0]) * tf.sigmoid(tf.Variable([0.5], dtype=tf.float32, trainable=True))
            self.theta = bound_theta[0] + (bound_theta[1] - bound_theta[0]) * tf.sigmoid(tf.Variable([0.35], dtype=tf.float32, trainable=True))
            ...
            self.loss = self.lossU + self.lossF + self.lossaux + self.lossconstr  #
        ...
        def net_h(self, x):
            ...
            # bound_h = [tf.constant(0.001, dtype=tf.float32), tf.constant(1, dtype=tf.float32)]
        ...
        def net_aux(self, x):
            ...
            f_d = tf.exp(-x * NN) * (-NN - x * NN_x) #case 1
    ...
    if __name__ == "__main__":
        ...
        # Residual/collocation points are generated once and held fixed during
        # the whole Bootstrap analysis.
        N_f = 2000
        np.random.seed(42)
        ...
        # Storage for the Bootstrap estimates
        ######################################################################
        ...
        loss_bootstrap = np.full(N_BOOTSTRAP, np.nan)
        bootstrap_summary = []

        
        # Residual Bootstrap: resample, refit and save each result
        ######################################################################
        for bootstrap_index, bootstrap_seed in enumerate(BOOTSTRAP_SEEDS):
            ...
            # The same initialization is used for all Bootstrap samples so
            # variability primarily reflects resampled-data uncertainty.
            tf.reset_default_graph()
            set_random_seed(MODEL_INITIALIZATION_SEED)
            total_records = []
            model = None
            ...
            try:
                ...
                loss_bootstrap[bootstrap_index] = float(final_loss)
        ...
        #  h(x) interval after interpolation to a common xstable grid
        ######################################################################
